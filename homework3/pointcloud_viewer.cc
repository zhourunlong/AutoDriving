...
void PointCloudViewer::keyPressEvent(QKeyEvent* event) {
  ...
  switch (key) {
    case Qt::Key_N:
      ...
      PointCloud pointcloud_ = ReadPointCloudFromTextFile(pointcloud_file), pointcloud_p, pointcloud_n;
      if (!file_index_) pointcloud_p = ReadPointCloudFromTextFile(pointcloud_files_[2]);
      else pointcloud_p = ReadPointCloudFromTextFile(pointcloud_files_[file_index_ - 1]);
      if (file_index_ == pointcloud_files_.size() - 1) pointcloud_n = ReadPointCloudFromTextFile(pointcloud_files_[file_index_ - 2]);
      else pointcloud_n = ReadPointCloudFromTextFile(pointcloud_files_[file_index_ + 1]);
      CHECK(!pointcloud_p.points.empty());
      CHECK(!pointcloud_.points.empty());
      CHECK(!pointcloud_n.points.empty());
      ...
      PointCloud frames[3];
      frames[0] = pointcloud_p;
      frames[1] = pointcloud_;
      frames[2] = pointcloud_n;
      detector_->GetGroundAndObstacles(frames, &eigen_points, &labels_);
